...
\begin{itemize}
    ...
    %Since the product of two non-zero elements is zero, this ring has zero-divisors and is \textbf{not} an integral domain. 
...
We now verify that $\Phi$ is surjective. Let $(u_1 + I_1, \ldots, u_k + I_k) \in R/I_1 \times \cdots \times R/I_k$ be arbitrary. 
...
\begin{example}
\begin{itemize}
\item In the ring $R = \mathbb{Z}_6$, the element $[2]$ is a zero-divisor because $[2] \cdot [3] = [6] = [0]$, and both $[2], [3] \neq [0]$. 

\item Let $R$ be unital commutative ring with polynomial ring $R[x]$, and let $p(x) = a(x)b(x) \in R[x]$ be a reducible polynomial. Then
...
A ring $R$ is a \textbf{domain} if it has no zero-divisors. A {\it unital, commutative} ring with no zero-divisors is called an \textbf{integral domain} (ID).
\end{definition}
...
{\bf Unless stated otherwise, we will assume $R$ is unital commutative from now on, and $R$ is automatically an integral domain if it has no zerodivisors}.
...
As hinted in the previous section, one would like to study various algebraic properties for integral domain $R$. We begin with defining what `prime' really means for $R$.

Rather than studying whether an element $r\in R$ is prime, it is better to define what it means for an ideal $I\triangleleft R$ to be prime. Indeed, we will define various properties on {\bf ideals of $R$}, rather than {\bf elements of $R$}. 
...
\end{center}
and further convert the definition of the property to relations between the elements $n_1, n_2, \dots$ in $\mathbb{Z}$. As an example, recall we defined
\begin{center}
    $I_1$, $I_2$ is coprime if $I_1 + I_2 = R$
\end{center}
which, in the prototype case when $R=\mathbb{Z}$, is translated into
\begin{center}
$\langle p\rangle,\langle q\rangle$ are coprime (as ideals) $\Leftrightarrow$ $\gcd(p,q)=1$ (as elements in $\mathbb{Z}$),
\end{center}
...
\begin{example}
\begin{itemize}
    \item Back into our prototype $R=\mathbb{Z}$. We would expect 
    \begin{center}
    $p \in \mathbb{Z}$ is a prime number if and only if $I = \langle p \rangle$ is a prime ideal. 
    \end{center}
    To see so, let $a,b\in \mathbb{Z}$. Then 
    $$ab\in \langle p\rangle\ \Leftrightarrow\  ab=pk\ (k\in \mathbb{Z})\ \Leftrightarrow p\mid ab\ \Leftrightarrow \ p\mid a\ or\; p\mid b\ \Leftrightarrow a\in \langle p \rangle\ or\ b\in \langle p \rangle,$$
    where $p\mid ab\ \Leftrightarrow \ p\mid a\ or\; p\mid b$ comes from the property that $p \in \mathbb{Z}$ is a prime number.
...
\begin{proposition}
Let $R$ be a commutative ring with unity and let $I \triangleleft R$. Then:
\begin{enumerate}
...
\]
Since $\mathbb{Z}$ is an integral domain, it follows that $\langle x \rangle$ is prime.

However, $\mathbb{Z}$ is \emph{not} a field, hence $\langle x \rangle$ is not maximal.
\end{example}
...
\begin{definition}
Let $R$ be a commutative ring with unity. An element $p \in R$ is called 
...
\begin{definition}
Let $a,b \in R$, where $R$ is a commutative ring with unity.  
We say that $a$ \textbf{divides} $b$ (written $a \mid b$) if
\[
\langle b \rangle \subseteq \langle a \rangle.
\]
(or equivalently \( b \in \langle a \rangle\). 
...
\noindent Recall: in any commutative ring with unity,
\[
I \text{ maximal } \Longrightarrow I \text{ prime}.
\]
The converse is \emph{false} in general (for example, $\langle x\rangle \subset \mathbb{Z}[x]$ is prime but not maximal).

\medskip
A natural goal is to find classes of integral domains $R$ where prime ideals are ``as close as possible'' to maximal ideals.
One particularly important class is when \emph{every} ideal is generated by one element.
...
\begin{enumerate}
    ...
    Consider the ideal $J=\langle 2,x\rangle \triangleleft \mathbb{Z}[x]$.
    If $J=\langle f(x)\rangle$ were principal, then $f(x)\mid 2$, hence $f(x)$ must be a constant $\pm 1,\pm 2$.
    But $x\in J$ forces $f(x)\mid x$, which is impossible for $f(x)=\pm 2$, and if $f(x)=\pm 1$ then $J=\mathbb{Z}[x]$,
    contradicting that $J$ is proper. Thus $J$ is not principal.
\end{enumerate}
\end{example}
...
\begin{proposition}[Prime ideals are maximal in a PID]
