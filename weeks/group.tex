...
\begin{itemize}
    ...
    which is an $S_n$-action on $X_n := \{1,2, \dots, n\}$. \item Similarly, one also has an injective $D_n$-action
    ...
    rotations on $\mathbb{R}^2$. Define a $G$-action on the unit sphere $X =S^2 := \{ (x,y,z) \in \mathbb{R}^3 : x^2 + y^2 + z^2 = 1 \}$ by:
    $$\sigma : SO(2) \to \mathrm{Aut}(S^2), \quad \quad \sigma\begin{pmatrix} \cos\theta & -\sin\theta  \\ \sin\theta & \cos \theta \end{pmatrix} := R_\theta,$$
 where $R_{\theta}({\bf x}) := \begin{pmatrix} \cos\theta & -\sin\theta & 0 \\ \sin\theta & \cos\theta & 0 \\ 0 & 0 & 1 \end{pmatrix}{\bf x}$ for ${\bf x} \in S^2$. So the $SO(2)$-action is precisely the rotation on the sphere along the polar axis:
\begin{center}
...
Let $G$ be a finite group acting on a finite set $X$. Then the number of orbits, denoted $|X/G| := |\{\mathcal{O}_x\ |\ x \in X\}|$, is equal to:
...
\begin{example}
\begin{itemize}
    \item $A_n \triangleleft S_n$ for all $n$. Indeed, for all $g \in G$, if $g \in A_n$ is even, then $gh, hg \in A_n$ for all $h \in A_n$ and $gA_n = A_n = A_ng$. Otherwise, if
    $g$ is odd, then one can easily check that $gA_n = A_ng$ is the collection of all odd permutations.
    \item More generally, if $H \le G$ and $[G:H] = 2$, then $H \triangleleft G$ (Homework).
    %. In this case, $G$ is the disjoint union $H \sqcup gH$ and also $H \sqcup Hg$ for any $g \notin H$. It follows that $gH = G \setminus H = Hg$.
...
\begin{example}
    We have $SL(n, \mathbb{R}) \triangleleft GL(n, \mathbb{R})$. Indeed, let $a \in GL(n, \mathbb{R})$ and $h \in SL(n, \mathbb{R})$. Then:
    \[ \det(a h a^{-1}) = \det(a) \det(h) \det(a)^{-1} = \det(a) \cdot 1 \cdot \frac{1}{\det(a)} = 1 \]
    Thus $a h a^{-1} \in SL(n, \mathbb{R})$, and the result follows from (ii) above.
\end{example}
...
We deal with the case when $r = 2$, and the general case follows by inductively from this case. 

Suppose $|G| = m n$
where $\gcd(m,n) = 1$ are coprime (e.g. $m = p_1^{a_1}$ and $n = p_2^{a_2}$). We want to show $G \cong G_1 \times G_2$ with $|G_1| = m$ and $|G_2| = n$. Let 
...
\begin{itemize}
    ...
    \item \textbf{Claim 2 -} $\phi: G^m \times G^n \to G$ given by $\phi(x, y) := xy$ is an isomorphism:
    ...
    But $x := g^{\beta n}$ satisfies $x^m = g^{
    \beta mn} = g^{\beta|G|} = e$ and similarly $y := g^{\alpha m}$ satisfies $y^n = e$. Therefore, $x \in G^m$ and $y \in G^n$, with
    $$\phi(x,y)=xy = g^{\beta n}g^{\alpha m} = g.$$
    
    \item \textbf{Claim 3 -} $|G^m| = m$ and $|G^n| = n$:

    We first show that $\gcd(|G^m|, n) = 1$: 
    Suppose on contrary that there exists a prime factor $q | n$ such that $q \mid |G^m|$. By Cauchy's Theorem (proved in Homework), there exists $x \in G^m$ of order $q$. By Bezout's Theorem, $\gamma q + \delta m = 1$ for some $\gamma, \delta \in \mathbb{Z}$. Then 
    $$x = x^{\gamma q + \delta m} = (x^q)^\gamma (x^m)^\delta = e \cdot e = e,$$ 
    contradicting $\text{ord}(x)=q$. Thus 
    $|G^m|$ is coprime with all prime factors of $n$.
    Analogously, $|G^n|$ is coprime with all prime factors of $m$. 
    
    Since $G^m \times G^n \cong G$, we have 
    \begin{center}
    $|G^m| \times |G^n| = mn \quad \Rightarrow \quad \begin{cases} |G^m|\ |\ mn \\ 
    |G^n|\ |\ mn \end{cases} \quad \Rightarrow \quad \begin{cases} |G^m|\ |\ m\\ |G^n|\ |\ n \end{cases}$,    
    \end{center}
...
\begin{itemize}
    ...
    \[ (\mu^i)^{p^{m-1}} = (\nu^p)^{p^{m-1}} = \nu^{p^m} = e \]
    since every element in $G$ has order $p^i \ |\ p^m$. This implies $\text{ord}(\mu^i) \mid p^{m-1}$, so $|\langle \mu^i \rangle| < p^m$, which means $\gcd(i, p^m) > 1$. Thus, $i = pj$ for some $j \in \mathbb{Z}$, and we have:
    \[ \nu^p = \mu^{pj}. \] 
    ...
    and $\overline{\mu} \in \overline{G}$ has maximal order $p^m$, one can apply  inductive hypothesis to obtain a subgroup $\bar{K} \le \bar{G}$ such that 
    $$\bar{G} \cong \langle \bar{\mu} \rangle \times \bar{K}.$$
    Let $\pi: G \to \overline{G}$ be defined by $\pi(g) := g\langle \nu\rangle$ (Exercise: Show that $\pi$ is a homomorphism), and $K := \pi^{-1}(\overline{K})$ (here we abuse notations and treat $\langle \overline{\mu}\rangle$, $\overline{K} \leq \overline{G}$ as subgroups). 
    ...
    Let $\mu^i \in \langle \mu \rangle \cap K$. Then $\pi(\mu^i) = \bar{\mu}^i \in \langle \bar{\mu}\rangle \cap \bar{K}$. Since $\bar{G} \cong \langle \bar{\mu} \rangle \times \bar{K}$ is an external direct product, $\langle\bar{\mu} \rangle \cap \bar{K} = \{e_{\overline{G}}\}$ and:
...
\begin{itemize}
    \item \textbf{By Step 1:} The group decomposes into:
    \[ G \cong G_8 \times G_9 \times G_5 \]
    where $|G_8| = 2^3$, $|G_9| = 3^2$, and $|G_5| = 5$ (these are called {\bf Sylow $p$-subgroups} ($p = 2,3,5$) of $G$).
